...
const SWARM_MODE_ENTER_REMINDER = [
  ...
  "- When you need to inspect progress, use `task_list` to see all active background subtasks, or `task_wait` to wait for a specific task and collect its final result. `AgentSwarm` itself waits up to 60 seconds and returns an `<agent_swarm_result>` summary with each subagent's outcome.",
].join("\n");
